...
public class VInsPersona extends JDialog implements ActionListener {
	...
	private JMenuItem mEspecifico;
	private JMenuItem mTotal;
	...
	private ContDatosInsertPer datos1;
	...
	public VInsPersona(VInserciones inserciones, boolean modal) {
		...
		btnAnadir.setBounds(595, 398, 89, 30);
		...
		btnVolver.setBounds(595, 434, 89, 30);
	...
	private void altaPersona() {
		if (rdbtnNewRadioButton.isSelected()) {
			...
			des.setFechaDes(stringDateTime(textFieldFechDes.getText()));
			...
			des.setAltura(stringFloat(textFieldAltura.getText()));
	...
	private LocalDateTime stringDateTime(String string) {
		LocalDateTime desaparicion = LocalDateTime.parse(string);
		return desaparicion;
	}

	private float stringFloat(String string) {
		float altura = Float.parseFloat(string);
		return altura;
	}
}
